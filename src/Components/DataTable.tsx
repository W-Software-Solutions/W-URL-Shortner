...
const DataTable: React.FunctionComponent<IDataTableProps> = (props) => {
    ...
    const copyToClipboard = async (url: string) => {
        try {
            await navigator.clipboard.writeText(`https://w-qjrm.onrender.com/${url}`);
            alert(`URL copied: https://w-qjrm.onrender.com/${url}`);
        } catch (error) {
            console.log(error);
        }
    };
    ...
    const deleteUrl = async (id: string) => {
        const response = await axios.delete(`https://w-qjrm.onrender.com/${id}`);
        updateReloadState();
    ...
    const renderTableData = () => {
        return data.map((item) => (
            <tr key={item._id} className="border-b text-white bg-gray-600 hover:bg-white hover:text-gray-800">
                ...
                <td className="px-6 py-3 break-words">
                    <Link to={`https://w-qjrm.onrender.com/${item.shortUrl}`} target="_blank" rel="noreferrer noopener">
                        {item.shortUrl}
                    </Link>
                </td>
        ...
        <div className="container mx-auto p-2 pb-10 bg-cyan-700">
            <div className="relative overflow-x-auto shadow-sm sm:rounded-lg">
                <table className="w-full table-auto text-sm text-left rtl:text-right text-gray-500">
                    <thead className="text-base sm:text-lg uppercase text-gray-50 bg-gray-700">
                        <tr>
